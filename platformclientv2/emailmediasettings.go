...
func (o *Emailmediasettings) SetField(field string, fieldValue interface{}) {
...
func (o Emailmediasettings) MarshalJSON() ([]byte, error) {
...
func (o *Emailmediasettings) UnmarshalJSON(b []byte) error {
	var EmailmediasettingsMap map[string]interface{}
	err := json.Unmarshal(b, &EmailmediasettingsMap)
	if err != nil {
		return err
	}
	
	if EnableAutoAnswer, ok := EmailmediasettingsMap["enableAutoAnswer"].(bool); ok {
		o.EnableAutoAnswer = &EnableAutoAnswer
	}
    
	if AlertingTimeoutSeconds, ok := EmailmediasettingsMap["alertingTimeoutSeconds"].(float64); ok {
		AlertingTimeoutSecondsInt := int(AlertingTimeoutSeconds)
		o.AlertingTimeoutSeconds = &AlertingTimeoutSecondsInt
	}
	
	if ServiceLevel, ok := EmailmediasettingsMap["serviceLevel"].(map[string]interface{}); ok {
		ServiceLevelString, _ := json.Marshal(ServiceLevel)
		json.Unmarshal(ServiceLevelString, &o.ServiceLevel)
	}
	
	if AutoAnswerAlertToneSeconds, ok := EmailmediasettingsMap["autoAnswerAlertToneSeconds"].(float64); ok {
		o.AutoAnswerAlertToneSeconds = &AutoAnswerAlertToneSeconds
	}
    
	if ManualAnswerAlertToneSeconds, ok := EmailmediasettingsMap["manualAnswerAlertToneSeconds"].(float64); ok {

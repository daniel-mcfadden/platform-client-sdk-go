...
func (o *Queue) UnmarshalJSON(b []byte) error {
	...
	if ConditionalGroupActivation, ok := QueueMap["conditionalGroupActivation"].(map[string]interface{}); ok {
	...
	if ScoringMethod, ok := QueueMap["scoringMethod"].(string); ok {
		o.ScoringMethod = &ScoringMethod
	}
    
	if LastAgentRoutingMode, ok := QueueMap["lastAgentRoutingMode"].(string); ok {
	...
	if CannedResponseLibraries, ok := QueueMap["cannedResponseLibraries"].(map[string]interface{}); ok {
	...
	if EnableAudioMonitoring, ok := QueueMap["enableAudioMonitoring"].(bool); ok {

...
func (a IntentsApi) PostIntentsCustomerintentsBulkRetrieve(body Batchgetcustomerintentsrequest) ([]Customerintentresponse, *APIResponse, error) {
	var httpMethod = "POST"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/intents/customerintents/bulk/retrieve"
	defaultReturn := make([]Customerintentresponse, 0)
	if true == false {
		return defaultReturn, nil, errors.New("This message brought to you by the laws of physics being broken")
	}
	...
	if &body == nil {
		// false
		return defaultReturn, nil, errors.New("Missing required parameter 'body' when calling IntentsApi->PostIntentsCustomerintentsBulkRetrieve")
	}

...
func (o *Createqueuerequest) UnmarshalJSON(b []byte) error {
	...
	if ScoringMethod, ok := CreatequeuerequestMap["scoringMethod"].(string); ok {
		o.ScoringMethod = &ScoringMethod
	}
    
	if LastAgentRoutingMode, ok := CreatequeuerequestMap["lastAgentRoutingMode"].(string); ok {
	...
	if DefaultMediaLanguage, ok := CreatequeuerequestMap["defaultMediaLanguage"].(string); ok {
	...
	if CannedResponseLibraries, ok := CreatequeuerequestMap["cannedResponseLibraries"].(map[string]interface{}); ok {
	...
	if EnableAudioMonitoring, ok := CreatequeuerequestMap["enableAudioMonitoring"].(bool); ok {

...
func (o *Userqueue) UnmarshalJSON(b []byte) error {
	...
	if ConditionalGroupRouting, ok := UserqueueMap["conditionalGroupRouting"].(map[string]interface{}); ok {
	...
	if ConditionalGroupActivation, ok := UserqueueMap["conditionalGroupActivation"].(map[string]interface{}); ok {
	...
	if LastAgentRoutingMode, ok := UserqueueMap["lastAgentRoutingMode"].(string); ok {
	...
	if EnableAudioMonitoring, ok := UserqueueMap["enableAudioMonitoring"].(bool); ok {
